...
private let rcLinear3 = rcLine(base: 1, tree: rcNode(1, [rcNode(2, [rcNode(3)])]))   // 커먼 3형태: 12.5M/25M/37.5M
private let rcNoEvo = rcLine(base: 20, tree: rcNode(20))                              // 커먼 1형태: 75M 단일
...
final class RareCandyStoreTests: XCTestCase {
    ...
    /// 사탕 XP(10M) < 최소 임계(12.5M) → 진화 못 시키는 케이스는 부분 진행(.progressed), 통계 불변.
    ...
    func testUseEvolvesWhenCrossingThreshold() async {
        let s = store(rcLinear3)
        await s.hatch(baseID: 1)
        s.applyUsage(5_000_000)   // stage0(12.5M) 잔여 7.5M ≤ 10M
        giveCandies(s, 1)
        let result = s.useRareCandy()
    ...
    /// [불변식] 사탕 1개 = 최대 1단계 — 임계 직전(12.4M)에서 써도 2단계 연쇄 안 됨.
    func testSingleCandyAdvancesAtMostOneStage() async {
        let s = store(rcLinear3)
        await s.hatch(baseID: 1)
        s.applyUsage(12_400_000)   // stage0 임계 직전
        giveCandies(s, 1)
        _ = s.useRareCandy()        // +10M → 22.4M: stage0(12.5M) 1회만, stage1(25M) 미달
    ...
    func testUseGraduatesFinalStage() async {
        ...
        s.applyUsage(70_000_000)   // 졸업 총량 75M 잔여 5M ≤ 10M
        giveCandies(s, 1)
        let result = s.useRareCandy()
    ...
    /// 데모 시나리오(구구 3형태, usedAtStage 10M, 사탕 3): 진화 → 부분성장 → 진화, 그 뒤 재고 0.
    func testSequentialCandyUseMatchesDemo() async {
        let s = store(rcLinear3)
        await s.hatch(baseID: 1)
        s.applyUsage(10_000_000)                       // stage0(12.5M) 도달 전
        giveCandies(s, 3)
        XCTAssertEqual(s.useRareCandy(), .evolved)     // 20M ≥12.5M → stage1, 이월 7.5M
        XCTAssertEqual(s.state.active?.stageIndex, 1)
        XCTAssertEqual(s.useRareCandy(), .progressed)  // 17.5M <25M → 부분성장
        XCTAssertEqual(s.state.active?.stageIndex, 1)
        XCTAssertEqual(s.useRareCandy(), .evolved)     // 27.5M ≥25M → stage2
